 ...
 * @author BuiDucMinh
 */
public class RomReponsitory {

    public List<Rom> getAllRom() {
        String sql = "select ID, MAROM, DUNGLUONG from ROM";
        try ( Connection con = DBConnections.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                list.add(new Rom(rs.getInt(1), rs.getString(2), rs.getInt(3)));
            }
            return list;
        } catch (Exception ex) {
            ex.printStackTrace(System.out);
            System.out.println("Loi Tai Reponsitory Rom");
        }
        return null;
    }

    public boolean addRom(Rom rom) {
        String sql = "insert into ROM(MAROM, DUNGLUONG) values (?,?)";
        int kiemTra = 0;
        try ( Connection con = DBConnections.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setObject(1, rom.getMaRom());
            ps.setObject(2, rom.getDungLuong());
    ...
    public boolean updateRom(Rom rom) {
        String sql = "update ROM set MAROM=?, DUNGLUONG=? where ID=?";
        int kiemTra = 0;
        try ( Connection con = DBConnections.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setObject(1, rom.getMaRom());
            ps.setObject(2, rom.getDungLuong());
            ps.setObject(3, rom.getId());
            kiemTra = ps.executeUpdate();
        } catch (Exception ex) {
            ex.printStackTrace(System.out);
    ...
    public boolean deteleRom(int id) {
