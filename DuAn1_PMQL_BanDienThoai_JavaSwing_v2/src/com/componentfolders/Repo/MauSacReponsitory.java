...
public class MauSacReponsitory {
        ...
        try ( Connection con = DBConnections.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.out.println("Loi Tai Reponsitory MauSac");
        }
        return null;
    }

    public boolean addMS(MauSac mauSac) {
    ...
    public boolean updateMS(MauSac mauSac) {
        String sql = "update MAUSAC set MAMAU=?, TENMAU=? where ID=?";
        int kiemTra = 0;
        try ( Connection con = DBConnections.getConnection();  PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setObject(1, mauSac.getMaMauSac());
            ps.setObject(2, mauSac.getTenMauSac());
    ...
    public boolean deteleMS(int id) {
